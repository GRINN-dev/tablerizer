...
export function generateMaterializedViewSQL(
  ...
  lines.push(`-- ========================================`);
  lines.push(`-- Materialized View: ${matview.schema_name}.${matview.matview_name}`);
  lines.push(`-- Generated by Tablerizer 🎲`);
  if (includeDate) {
    lines.push(`-- Date: ${new Date().toISOString()}`);
  }
  ...
  lines.push("/*");
  lines.push(`  MATERIALIZED VIEW DOCUMENTATION: ${matview.schema_name}.${matview.matview_name}`);
  ...
  lines.push(`  Owner: ${matview.owner}`);
  lines.push(`  Status: ${matview.is_populated ? 'Populated' : 'Not Populated'}`);
  lines.push("");
